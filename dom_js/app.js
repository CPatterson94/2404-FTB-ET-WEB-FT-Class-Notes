...
function calculateTotalInventory() {
    ...
    return items.reduce((acc, i)=>acc+i.inventory, 0);
...
function calculateAllInventoryPrice() {
    ...
    const prices =[];
    items.forEach((i)=>{
        prices.push(i.inventory* i.price)
    })
    return prices.reduce((acc, cv)=>acc+cv, 0);
